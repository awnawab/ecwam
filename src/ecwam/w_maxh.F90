...
REAL(KIND=JWRB), DIMENSION(4) :: TLGS, ACFS, TLGS_SUM(4)
...
DO IJ = KIJS, KIJL
  ...
  DO M = 1, NFRE
    TLGS_SUM(1) = TLGS_SUM(1) + COS( OMEGA(M)*TLGS(1) ) * TEMPDFIM(IJ,M)
    TLGS_SUM(2) = TLGS_SUM(2) + COS( OMEGA(M)*TLGS(2) ) * TEMPDFIM(IJ,M)
    TLGS_SUM(3) = TLGS_SUM(3) + COS( OMEGA(M)*TLGS(3) ) * TEMPDFIM(IJ,M)
    TLGS_SUM(4) = TLGS_SUM(4) + COS( OMEGA(M)*TLGS(4) ) * TEMPDFIM(IJ,M)
  ENDDO
