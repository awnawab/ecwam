...
! ----------------------------------------------------------------------
      ...
      IF (LOUNIT) THEN
        IUNIT=IWAM_GET_UNIT(IU06, FILENAME(1:LFILE) , 'w', 'u', 0, 'READWRITE', LSTREAM=LSTREAM)
      ELSE
        IUNIT=IWAM_GET_UNIT(IU06, FILENAME(1:LFILE) , 'a', 'u', 0, 'READWRITE', LSTREAM=LSTREAM)
      ENDIF
...
!     BUT THE SINGLE BINARY INPUT FILES SHOULD BE IN THE OLD MAPPING
        WRITE(IUNIT) FL(IJINF:IJSUP,KINF:KSUP,MINF:MSUP)
      ENDIF
