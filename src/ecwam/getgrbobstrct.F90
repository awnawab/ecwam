...
IF ( LSUBGRID ) THEN
 ...
 &                       CDSTRING='GETGRBOBSTRCT: RAW GRIB BATCH')
    ...
    ! ---- ALL PES: DECODE AND SCATTER EACH ANGLE ----
    DO K = 1, NANG_OBS
      ...
      IF (IS > 0) THEN
        ...
        ! DECOMPRESS GRIB VALUES (THE EXPENSIVE OPERATION - NOW DONE IN PARALLEL ON ALL PES)
        ...
        ELSE IF (SIZE(VALUES) < NUMBEROFVALUES) THEN
          DEALLOCATE(VALUES)
          ALLOCATE(VALUES(NUMBEROFVALUES))
        ENDIF
        ...
        IF (ASSOCIATED(OBS_TARGET)) THEN
          ...
          CALL GSTATS(1498,0)
!$OMP     PARALLEL DO SCHEDULE(STATIC) PRIVATE(JKGLO, KIJS, KIJL, IJ, IX, IY, L)
