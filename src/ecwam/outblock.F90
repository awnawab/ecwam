...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        DO IJ=KIJS,KIJL
          BOUT(IJ,ITG)=LOCAL_MOD(DEG*BOUT(IJ,ITG)+180._JWRB,360._JWRB)
        ENDDO
      ENDIF
...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        DO IJ=KIJS,KIJL
          BOUT(IJ,ITOBOUT(5))=LOCAL_MOD(DEG*WDWAVE(IJ)+180._JWRB,360._JWRB)
        ENDDO
      ENDIF
...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        DO IJ=KIJS,KIJL
          BOUT(IJ,ITOBOUT(13))=LOCAL_MOD(DEG*THWISEA(IJ)+180._JWRB,360._JWRB)
        ENDDO
      ENDIF
...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        DO IJ=KIJS,KIJL
          BOUT(IJ,ITOBOUT(14))=LOCAL_MOD(DEG*THSWELL(IJ)+180._JWRB,360._JWRB)
        ENDDO
      ENDIF
...
!     SURFACE STOKES DRIFT U and V
      ...
      DO ITR=1,NTRAIN
        ...
        IF (IPFGTBL(43 + (ITR-1)*3) /= 0) THEN
          DO IJ=KIJS,KIJL
            BOUT(IJ,ITOBOUT(43 + (ITR-1)*3))=LOCAL_MOD(DEG*THTRAIN(IJ,ITR)+180._JWRB,360._JWRB)
          ENDDO
        ENDIF
...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        DO IJ=KIJS,KIJL
          BOUT(IJ,ITOBOUT(55 + 3*NTRAIN))=LOCAL_MOD(DEG*FLD2(IJ)+180._JWRB,360._JWRB)
        ENDDO
      ENDIF
...
IF (LHOOK) CALL DR_HOOK('OUTBLOCK',1,ZHOOK_HANDLE)
      ...
      FUNCTION LOCAL_MOD(A, P) RESULT(RES)
 
        REAL(KIND=JWRB), INTENT(IN) :: A, P
        REAL(KIND=JWRB) :: RES
