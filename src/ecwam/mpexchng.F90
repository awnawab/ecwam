...
!----------------------------------------------------------------------
      ...
      REAL(KIND=JWRB), POINTER, CONTIGUOUS :: ZCOMBUFS(:,:) => NULL()
      REAL(KIND=JWRB), POINTER, CONTIGUOUS :: ZCOMBUFR(:,:) => NULL()
      CLASS(FIELD_2RB), POINTER :: F_ZCOMBUFS => NULL()
      CLASS(FIELD_2RB), POINTER :: F_ZCOMBUFR => NULL()
#ifdef WITH_GPU_AWARE_MPI
      TYPE(MPI_COMM) :: ICOMM
...
!----------------------------------------------------------------------
      ...
      CALL FIELD_NEW(F_ZCOMBUFS, UBOUNDS=[NBUFMAX, NGBTOPE], PERSISTENT=.TRUE., DELAYED=.TRUE.)
      CALL FIELD_NEW(F_ZCOMBUFR, UBOUNDS=[NBUFMAX, NGBFROMPE], PERSISTENT=.TRUE., DELAYED=.TRUE.)

#ifdef _OPENACC
      CALL F_ZCOMBUFS%GET_DEVICE_DATA_WRONLY(ZCOMBUFS)
#else
      CALL F_ZCOMBUFS%GET_HOST_DATA_RDWR(ZCOMBUFS)
#endif
...
#ifdef WITH_GPU_AWARE_MPI
      CALL F_ZCOMBUFR%GET_DEVICE_DATA_WRONLY(ZCOMBUFR)
#else
      CALL F_ZCOMBUFR%GET_HOST_DATA_RDWR(ZCOMBUFR)
#endif
...
CALL F_ZCOMBUFS%GET_HOST_DATA_RDONLY(ZCOMBUFS)
#endif
...
#ifndef WITH_GPU_AWARE_MPI
CALL F_ZCOMBUFR%GET_DEVICE_DATA_RDONLY(ZCOMBUFR)
#endif
...
!$OMP END PARALLEL DO
      ...
      CALL FIELD_DELETE(F_ZCOMBUFS)
      CALL FIELD_DELETE(F_ZCOMBUFR)
      NULLIFY(ZCOMBUFS)
      NULLIFY(ZCOMBUFR)
