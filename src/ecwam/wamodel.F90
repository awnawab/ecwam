...
#ifdef WAM_GPU
          CALL VARS_4D%F_FL1%GET_DEVICE_DATA_RDONLY(FL1)
          CALL MIJ%F_PTR%GET_DEVICE_DATA_RDONLY(MIJ_PTR)
          CALL VARS_4D%F_XLLWS%GET_DEVICE_DATA_RDONLY(XLLWS)

          CALL OUTBS_LOKI_GPU (MIJ_PTR, FL1, XLLWS, &
...
!           OUTPUT POINT SPECTRA (not usually used at ECMWF)
            ...
            CALL VARS_4D%SYNC_HOST_RDONLY(FL1=.TRUE.)
            CALL FF_NOW%SYNC_HOST_RDONLY(AIRD=.TRUE., WDWAVE=.TRUE., CICOVER=.TRUE., WSWAVE=.TRUE.,  &
            & WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
...
!                 --------------------------------------
            IF ( .NOT.LGRIBOUT .OR. LDWRRE ) THEN
              ...
              & WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
              & CHRNCK=.TRUE., CITHICK=.TRUE., USTRA=.TRUE., VSTRA=.TRUE.)
              CALL WVENVI%SYNC_HOST_RDONLY(DEPTH=.TRUE., DELLAM1=.TRUE., COSPHM1=.TRUE., UCUR=.TRUE., VCUR=.TRUE., &
...
!       ----------------------------------------------------
        IF (LWNEMOCOU .AND. (.NOT.LWCOU)) THEN
          ...
          IF (MOD(NEMOWSTEP,NEMOFRCO) == 0) THEN
            ...
            CALL WAM2NEMO%SYNC_HOST_RDONLY(NEMOUSTOKES=.TRUE., NEMOVSTOKES=.TRUE., NEMOSTRN=.TRUE.,  &
            & NPHIEPS=.TRUE., NTAUOC=.TRUE., NSWH=.TRUE., NMWP=.TRUE., NEMOTAUX=.TRUE.,  &
