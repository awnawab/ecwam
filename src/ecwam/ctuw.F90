...
#else
      !$acc parallel loop present(KXLT)
#endif
      DO K=1,NANG
        KP1 = K+1
        IF (KP1 > NANG) KP1 = 1
...
!       =1 IF A AND B HAVE THE SAME SIGN, 0 OTHERWISE
      ...
      SUBROUTINE CTUW_OFFLOAD()
        CALL BLK2GLO%F_KXLT%GET_DEVICE_DATA_RDONLY(KXLT)
        CALL BLK2GLO%F_IXLG%GET_DEVICE_DATA_RDONLY(IXLG)
      END SUBROUTINE CTUW_OFFLOAD

      SUBROUTINE CTUW_SET_POINTERS()
        CALL BLK2GLO%F_KXLT%GET_HOST_DATA_RDONLY(KXLT)
        CALL BLK2GLO%F_IXLG%GET_HOST_DATA_RDONLY(IXLG)
