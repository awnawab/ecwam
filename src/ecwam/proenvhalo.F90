...
!#ifdef OMPGPU
!!$omp target update from(WAVNUM,CGROUP,OMOSNH2KD,DELLAM1,COSPHM1,DEPTH,UCUR,VCUR)
!#endif
...
!      CALL PROENVHALO_OFFLOAD()
!#ifdef OMPGPU
!!$omp target update to(BUFFER_EXT)
!#endif
...
IF (LHOOK) CALL DR_HOOK('PROENVHALO',1,ZHOOK_HANDLE)
      ...
      SUBROUTINE PROENVHALO_OFFLOAD()
         CALL WVPRPT_LAND%F_WAVNUM%GET_DEVICE_DATA_RDWR(WAVNUM_LAND)
         CALL WVPRPT_LAND%F_CGROUP%GET_DEVICE_DATA_RDWR(CGROUP_LAND)
         CALL WVPRPT_LAND%F_OMOSNH2KD%GET_DEVICE_DATA_RDWR(OMOSNH2KD_LAND)
      END SUBROUTINE PROENVHALO_OFFLOAD

      SUBROUTINE PROENVHALO_SET_POINTERS()
         CALL WVPRPT_LAND%F_WAVNUM%GET_HOST_DATA_RDWR(WAVNUM_LAND)
         CALL WVPRPT_LAND%F_CGROUP%GET_HOST_DATA_RDWR(CGROUP_LAND)
         CALL WVPRPT_LAND%F_OMOSNH2KD%GET_HOST_DATA_RDWR(OMOSNH2KD_LAND)
